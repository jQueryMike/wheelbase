...
const getStylingClasses = () => {
  ...
  return Array.from(
    new Set([
      ...
      ...Object.values(mappings.gridColumn)
        .map((o) => Object.entries(o).map(([k, v]) => `${k}:${v}`))
        .flat(),
      ...Object.values(mappings.gridGap)
        .map((o) => Object.entries(o).map(([k, v]) => `${k}:${v}`))
        .flat(),
      ...Object.values(mappings.spacing)
        .map((o) => Object.values(o))
        .flat()
        .map((o) => Object.entries(o).map(([k, v]) => spacingVariants.map((prefix) => `${k}:${prefix}-${v}`)))
        .flat(2),
      ...Object.values(mappings.fontSize)
        .map((o) => Object.values(o))
        .flat()
        .map((o) => Object.entries(o).map(([k, v]) => `${k}:${v}`))
        .flat(),
