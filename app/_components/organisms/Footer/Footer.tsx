...
const Footer = async ({
  ...
  const buildConfigForItem = (item?: any) =>
    item?.content ? buildConfig(item.content) : undefined;
  const socialContent = socials?.items?.items?.map(buildConfigForItem) || [];

  const chosenSocials = socialContent.map(
    ({ icon, socials: socialItem, styling: socialStyling }: any) => ({
      icon,
      socials: socialItem?.socials?.[0]
        ? buildConfig(socialItem.socials[0])
        : undefined,
  ...
  const companyInfoContent =
    companyInfo?.items?.items?.map(buildConfigForItem) || [];
  const chosenCompanyInfo = {
    styling: companyInfoContent[0]?.styling || {},
    items:
      companyInfoContent[0]?.companyInfoItems.map((x: any) => buildConfig(x)) ||
      [],
  };
  return (
    <BaseComponent
