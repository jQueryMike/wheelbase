...
export default function buildSEO(page: any) {
  ...
  if (page.properties.metaDescription)
    seo.description = page.properties.metaDescription;
  ...
  if (
    page.properties.twitterHandle ||
    page.properties.twitterSite ||
    page.properties.twitterCardType
  ) {
    seo.twitter = {
  ...
  if (page.properties.metaKeywords) {
    seo.additionalMetaTags = [
      { name: "keywords", content: page.properties.metaKeywords },
    ];
  }
