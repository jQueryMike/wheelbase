import ClassesBuilder, { ClassesProperty, tw } from '@utilities/ClassesBuilder';

import { HeaderClasses } from '../Header';
import HeaderVariant from './HeaderVariant';

const location = 'Header/variants/5';

let classes: HeaderClasses<ClassesProperty> = {};

classes = {
  root: {
    default: tw`fixed left-0 right-0 top-0 z-50 bg-body px-8 py-1 transition duration-150`,
    md: tw`md:px-12 md:py-2`,
    lg: tw`lg:px-16`,
  },
  rootScrolled: tw`shadow-xl`,
  spacer: tw`h-20`,
  spacerScrolled: tw``,
  container: tw`container mx-auto flex h-20 items-center justify-between`,
  containerScrolled: tw``,
  drawerNavigationContainer: {
    default: tw`mr-10 block`,
    md: tw`md:hidden`,
  },
  logoContainer: tw`relative h-16 w-auto [&_img]:h-16 [&_img]:w-auto`,
  contentAreaContainer: tw`flex flex-grow items-center justify-end space-x-10`,
};

const headerVariant: HeaderVariant = { classes: new ClassesBuilder({ location, classes }).classes };

export default headerVariant;
